...
**3. Стандартная энергия Гиббса реакции (ΔrG°):**
...
Установление связи между различными макроскопическими свойствами системы, находящейся в равновесии. Применительно к электрохимическим системам – это установление связи между электрическими характеристиками (ЭДС, потенциал) и термодинамическими функциями состояния (ΔG, ΔH, ΔS) для суммарной реакции в цепи.

**2. Связь ЭДС с энергией Гиббса (ΔG):**
Максимальная полезная работа, которую может совершить система при P,T=const, равна убыли энергии Гиббса: `A_max = -ΔG`. В электрохимической цепи максимальная электрическая работа равна `A_эл = nFE`. Следовательно:
`ΔG = -nFE`
*   Стандартная связь: `ΔG⁰ = -nFE⁰`

**3. Связь ЭДС с энтропией (ΔS):**
Используется уравнение Гиббса-Гельмгольца: `ΔG = ΔH - TΔS` и его производная `(∂(ΔG)/∂T)p = -ΔS`.
Подставляя `ΔG = -nFE`:
`(∂(-nFE)/∂T)p = -nF(∂E/∂T)p = -ΔS`
`ΔS = nF(∂E/∂T)p`
*   **Температурный коэффициент ЭДС (`∂E/∂T)p`** позволяет определить изменение энтропии в реакции.

**4. Связь ЭДС с энтальпией (ΔH):**
Подставляем выражения для `ΔG` и `ΔS` в уравнение `ΔH = ΔG + TΔS`:
`ΔH = -nFE + T * nF(∂E/∂T)p`
`ΔH = nF[T(∂E/∂T)p - E]`
*   Зная ЭДС и ее температурный коэффициент, можно рассчитать все основные термодинамические функции реакции (ΔG, ΔH, ΔS), не прибегая к калориметрии. Это один из важнейших методов химической термодинамики.